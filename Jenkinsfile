        ...
        stage('Setup K3d Environment') {
            steps {
                script {
                    ...
                    // Run the modified setup-cluster.sh script
                    sh '''
                    chmod +x upload_cluster.sh
                    ./upload_cluster.sh
                    ...
                    echo "Checking if kubeconfig exists: $(ls -la ${KUBECONFIG_PATH})"
                    
                    # Ensure kubeconfig is properly set for all subsequent steps
                    cp ${KUBECONFIG_PATH} ${WORKSPACE}/.kube/config || mkdir -p ${WORKSPACE}/.kube && cp ${KUBECONFIG_PATH} ${WORKSPACE}/.kube/config
                    
                    # Validate connection to cluster
                    KUBECONFIG=${KUBECONFIG_PATH} kubectl get nodes -o wide || {
                        echo "Failed to connect to cluster. Printing diagnostic information:"
            ...
            steps {
                withEnv(["KUBECONFIG=${KUBECONFIG_PATH}"]) {
                    ...
                    if [ -f k8s/kustomization.yaml ]; then
                        ...
                        # Apply individual files if no kustomization exists
                        kubectl apply -f k8s/config-and-secrets.yaml || true
...
EOF
    ...
    post {
        always {
            ...
            // Archive relevant logs and configs for debugging
            sh '''
            mkdir -p ${WORKSPACE}/logs
            kubectl get nodes > ${WORKSPACE}/logs/nodes.txt || true
            kubectl get pods -A > ${WORKSPACE}/logs/pods.txt || true
            kubectl describe pods > ${WORKSPACE}/logs/pods-describe.txt || true
            docker ps -a > ${WORKSPACE}/logs/docker-ps.txt || true
            cp ${KUBECONFIG_PATH} ${WORKSPACE}/logs/kubeconfig.txt || true
            '''
