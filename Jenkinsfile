...
pipeline {
    ...
    stages {
        stage('Cleanup') {
            steps {
                echo "מנקה מקלאסטרים קודמים..."
                sh 'minikube delete -p ${CLUSTER_NAME} || true'
            }
        }
        
        stage('הקמת קלאסטר') {
            steps {
                echo "יוצר קלאסטר מיניקיוב עם שני נודים..."
                sh 'minikube start --nodes=2 -p ${CLUSTER_NAME}'
                
                // וידוא שהקלאסטר עובד תקין
                sh 'kubectl get nodes'
            }
        }
        
        stage('הגדרת שמות לשרתים') {
            steps {
                echo "מגדיר שם לשרת הראשון: server1-manager..."
                sh 'kubectl label nodes ${CLUSTER_NAME} kubernetes.io/hostname=server1-manager --overwrite'
                
                echo "מגדיר שם לשרת השני: server2..."
                sh 'kubectl label nodes ${CLUSTER_NAME}-m02 kubernetes.io/hostname=server2 --overwrite'
                
                echo "מציג נודים והלייבלים שלהם:"
                sh 'kubectl get nodes --show-labels'
            }
        }
        
        stage('יישום קונפיגורציה') {
            steps {
                echo "הקלאסטר מוכן עם שני שרתים:"
                echo "1. server1-manager - עבור הדאטאבייס"
                echo "2. server2 - עבור הבאקאנד והפרונטאנד"
                
                // אם יש קבצי קונפיגורציה בתיקייה הנוכחית, מיישם אותם
                script {
                    def configFiles = sh(script: 'find . -maxdepth 1 -name "*.yaml" -o -name "*.yml" | wc -l', returnStdout: true).trim()
                    if (configFiles != '0') {
                        echo "מיישם קבצי קונפיגורציה..."
                        sh 'kubectl apply -f .'
                    } else {
                        echo "לא נמצאו קבצי קונפיגורציה ליישום"
                    }
                }
            }
        }
