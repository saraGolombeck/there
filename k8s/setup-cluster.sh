...
# יצירת קלאסטר חדש עם פורט קבוע לשרת ה-API
k3d cluster create my-cluster --api-port 6550 --agents 1 --registry-use registry.k3d:5000 -p "8080:80@loadbalancer"

# ודא שה-kubeconfig נוצר במיקום הנכון (זה מתייחס למשתנה הסביבה KUBECONFIG)
k3d kubeconfig get my-cluster > ${KUBECONFIG}
chmod 600 ${KUBECONFIG}

# קבלת שמות הצמתים  
MASTER_NODE=$(kubectl --kubeconfig=${KUBECONFIG} get nodes -o jsonpath='{.items[0].metadata.name}')
WORKER_NODE=$(kubectl --kubeconfig=${KUBECONFIG} get nodes -o jsonpath='{.items[1].metadata.name}')
